...
const EditPaymentDialog = ({ open, onOpenChange, onSubmit, payment }) => {
  const {
    register,
    control,
    handleSubmit,
  ...
  useEffect(() => {
    if (payment) {
      reset({
        home_id: payment.home_id.toString(),
        ocupant_id: payment.ocupant_id.toString(),
        dues_type_id: payment.dues_type_id.toString(),
  ...
  const ocupantOptions =
    ocupantsData?.data?.map((ocupant) => ({
      value: ocupant.id.toString(),
      label: ocupant.name,
    })) || [];

  const duesTypeOptions =
    duesTypesData?.data?.map((duesType) => ({
  ...
  return (
    <DiFormDialog
      ...
      description="Ubah detail pembayaran yang dipilih."
    >
