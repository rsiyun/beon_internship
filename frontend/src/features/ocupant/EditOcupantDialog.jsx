...
const EditOcupantDialog = ({ open, onOpenChange, onSubmit, ocupant }) => {
  ...
  const {
    ...
    defaultValues: {
      name: ocupant?.name || "",
      phone: ocupant?.phone || "",
      resident_status: ocupant?.resident_status || "",
      is_married: ocupant?.is_married || "",
      identity_card: null,
    },
  });
  useEffect(() => {
    if (ocupant) {
      reset({
        ...
        phone: ocupant.phone,
        resident_status: ocupant.resident_status,
        is_married: Number(ocupant.is_married).toString(),
        identity_card: null,
      });
      if (ocupant.identity_card) {
        setImagePreview(ocupant.identity_card);
      }
    }
  }, [ocupant, reset]);
  ...
  return (
        <DiFormDialog onOpenChange={onOpenChange} open={open} title="Edit Penghuni" description="Isi detail penghuni yang akan dirubah.">
...
export default EditOcupantDialog;
